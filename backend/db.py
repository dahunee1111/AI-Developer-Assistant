...
def ensure_column_exists(cursor, table_name: str, column_name: str, alter_sql: str):
    """
    테이블에 컬럼이 존재하는지 확인하고, 없으면 추가합니다.
    
    Args:
        cursor: 데이터베이스 커서
        table_name: 테이블 이름
        column_name: 컬럼 이름
        alter_sql: ALTER TABLE 쿼리
    """
    try:
        ...
        logger.warning(f"컬럼 존재 여부 확인 중 오류: {table_name}.{column_name} - {e}")
...
def hash_password(password: str) -> str:
    ...
    try:
        ...
        raise ValueError("비밀번호 처리 중 오류가 발생했습니다.")
...
def user_exists(cursor, user_id: int) -> bool:
    ...
    try:
        ...
        logger.error(f"사용자 존재 여부 확인 실패: {e}")
        return False
...
def add_points(user_id: int, amount: int, reason: str) -> bool:
    ...
    try:
        ...
        logger.error(f"포인트 추가 실패 (user_id={user_id}): {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def get_total_points(user_id: int) -> int:
    ...
    try:
        ...
        logger.error(f"포인트 조회 실패 (user_id={user_id}): {e}")
        return 0
    finally:
        if conn:
            conn.close()
...
def spend_points(user_id: int, amount: int, reason: str) -> bool:
    ...
    Returns:
        성공하면 True, 포인트 부족이면 False
    """
    try:
        total = get_total_points(user_id)
        if total < amount:
            logger.warning(f"포인트 부족 (user_id={user_id}, required={amount}, available={total})")
            return False

        return add_points(user_id, -amount, reason)
    except Exception as e:
        logger.error(f"포인트 사용 실패 (user_id={user_id}): {e}")
        return False


def ensure_profile_custom_row(user_id: int) -> bool:
    """
    사용자의 프로필 커스터마이징 행을 생성합니다 (없으면).
    ...
    try:
        ...
        logger.error(f"프로필 커스터마이징 행 생성 실패 (user_id={user_id}): {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def seed_exam_data():
 ...
 """,
        ...
        conn.commit()
        logger.info("✅ 시험 데이터 초기화 완료")
    except sqlite3.Error as e:
        logger.error(f"시험 데이터 초기화 실패: {e}")
    finally:
        if conn:
            conn.close()
...
def seed_shop_items():
    ...
    try:
        ...
        conn.commit()
        logger.info("✅ 상점 아이템 초기화 완료")
    except sqlite3.Error as e:
        logger.error(f"상점 아이템 초기화 실패: {e}")
    finally:
        if conn:
            conn.close()
